...
A clicked root and the selected quality produce a persistent accent-colored triad, with the root distinguished by a white border. Independently, up to 31 computer keys provide literal piano semantics: each held key adds a momentary white glow, white held-note edges, and sustained audio without replacing the selected triad. Repeated octaves share one node. Root, First, and Second audition concrete MIDI voicings through one persistent Rust/CPAL audio process.

Quality is an optional exclusive selection. Clicking the active Major, Minor, Diminished, or Augmented button toggles it off, clears the persistent chord while preserving the selected root, and disables inversion auditions until a quality is chosen again. Clicking another quality switches directly to it.

The center acts as an analytical lens rather than only a chord label. It displays the spelled chord tones, scale-degree roles, and semitone coordinates together—for example `C · E · G`, `1 · 3 · 5`, and `0 · 4 · 7 st`. Active tonal nodes show their role relative to the interpreted root (`1`, `♭3` or `3`, and `♭5`, `5`, or `♯5`). When held notes form a recognized triad, their momentary white layer uses that played chord's roles; otherwise the secondary node label remains its computer-key binding. Held pitch classes also produce temporary numerical markers at their chromatic positions inside the tonal circle. These coordinates run clockwise from `0` at C at the top through `11` at B and directly mirror the simultaneously activated outer-ring sectors.
